...
export const startCommand = new Command('start')
    ...
    try {
        ...
        const payload = { error: message, phase: 'start' };
    ...
    process.stderr.write(`[browser-base] session live on context '${info.context}' (debug: ${info.debugUrl})\n` +
    ...
    if (opts.httpPort) {
        ...
        httpServer.listen(opts.httpPort, opts.httpHost, () => {
            process.stderr.write(`[browser-base] HTTP status server listening on http://${opts.httpHost}:${opts.httpPort}/status\n`);
        });
    }
    await new Promise(() => { });
});
