  ...
  .action(async (opts) => {
    ...
    if (!['pi-agent', 'claude-code', 'both'].includes(agent)) {
      process.stderr.write(
        JSON.stringify({ error: `Unknown --agent value '${agent}'. Use pi-agent, claude-code, or both.` }) + '\n'
      );
      process.exit(1);
    }
    ...
    const actions: Array<{ action: string; target: string; status: 'would-write' | 'wrote' | 'skipped' }> = [];
    ...
    if (await exists(defaultContextDir)) {
      ...
      if (!dryRun) await fs.mkdir(defaultContextDir, { recursive: true });
    }
...
async function detectProjectRoot(startDir: string): Promise<string> {
  ...
  if (await hasAnyMarker(dir)) return dir;

  // Walk up.
  while (true) {
    if (await hasAnyMarker(dir)) return dir;
    const parent = path.dirname(dir);
    if (parent === dir || parent === root) return path.resolve(startDir);
    dir = parent;
  }
}
...
function buildNextSteps(opts: { contextsDir: string; defaultContext: string }): string[] {
