...
const DEFAULTS: Omit<ProjectConfig, 'contextDir' | 'browserPath'> = {
...
async function readJsonConfig(configPath: string): Promise<Partial<ProjectConfig> | null> {
  try {
...
export async function loadProjectConfig(opts: {
...
} = {}): Promise<{ config: ProjectConfig; configPath: string | null }> {
  ...
  let userConfig: Partial<ProjectConfig> | null = null;
  ...
  const headful =
    merged.headful ?? (env[ENV_KEYS.headful] === '1' || env[ENV_KEYS.headful] === 'true');

  const verbose = ((): 0 | 1 | 2 => {
    if (typeof merged.verbose === 'number') return merged.verbose;
    const raw = env[ENV_KEYS.verbose];
    if (!raw) return DEFAULTS.verbose;
  ...
  const chromePort = (() => {
    if (typeof merged.chromePort === 'number') return merged.chromePort;
    const raw = env[ENV_KEYS.chromePort];
    if (!raw) return DEFAULTS.chromePort;
    const n = parseInt(raw, 10);
 ...
 */
export function toResolvedConfig(project: ProjectConfig) {
  return {
